...
$check = $conn -> query("SELECT * FROM `files` WHERE `owner` = '" . $_SESSION['id'] . "' AND `remove` = 'false'");
		...
		if ($data['size'] == 0) {
			...
			echo "</div>
				<div class='timeintel inline'><span class='text'>CREATED: " .$data['created']. " | EXPIRES: " .$data['expired']. " </span></div><div class='filesactions inline'>
					...
					<span style='float:right;' class='text'>|</span><button class='filebutton' onclick=\"fileManipulation('PASS', " .$data['id']. ");\">SET PASS</button>
					<span style='float:right;' class='text'>|</span><button class='filebutton' onclick=\"fileManipulation('PASSOUT', " .$data['id']. ");\">REMOVE PASS</button>
