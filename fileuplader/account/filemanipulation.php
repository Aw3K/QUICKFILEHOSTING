...
if (!empty($mode) and isset($mode)) {
	$fid = htmlspecialchars($_GET['fileid']);
	if (empty($fid)) {
		if ($mode != "RECA" && $mode != "DELA") exit ("That files dont belong to current user.");
	} else {
	...
	if ($mode == "DEL") {
		...
		$pass = mysqli_real_escape_string($conn, htmlspecialchars($_GET['pass']));
		$pass = md5($pass);
		$action = $conn -> query("UPDATE `files` SET `pass` = '$pass' WHERE `id` = '$fid'");
		if ($action) exit ("SUCCESS");
		else exit ("Cound't set password. Try to reload page or contact support.");
	}
	else if ($mode == "PASSOUT") {
		$action = $conn -> query("UPDATE `files` SET `pass` = '' WHERE `id` = '$fid'");
		if ($action) exit ("SUCCESS");
		else exit ("Cound't remove password. Try to reload page or contact support.");
	}
	else if ($mode == "REC") {
		$action = $conn -> query("UPDATE `files` SET `remove` = 'false',`locked` = 'false' WHERE `id` = '$fid' AND `remove` = 'true'");
		if ($action) exit ("SUCCESSR");
		else exit ("Cound't recover file. Try to reload page or contact support.");
	}
	else if ($mode == "RECA") {
		$action = $conn -> query("UPDATE `files` SET `remove` = 'false',`locked` = 'false' WHERE `owner` = '" . $_SESSION['id'] . "' AND `remove` = 'true'");
		if ($action) exit ("SUCCESSR");
		else exit ("Cound't recover all files possible. Try to reload page or contact support.");
	}
